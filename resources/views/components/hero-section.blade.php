<!-- Hero Section -->
  <section id="beranda" class="bg-gradient-to-r from-[#1e3a8a] to-[#000000] relative min-h-screen px-4 sm:px-6 lg:px-12 py-20">
    <div class="relative z-10 max-w-7xl mx-auto flex flex-col lg:flex-row items-center lg:items-start justify-between gap-8">

      <!-- Konten Kiri (Teks) -->
      <div class="flex-1 text-white space-y-6 px-2 sm:px-0 lg:mt-30 max-w-full">
        <h1 class="text-3xl sm:text-4xl lg:text-5xl font-bold leading-snug break-words">
          Temukan Pakaian Terbaik<br>Untuk Hari Terbaik.
        </h1>
        <p class="text-sm sm:text-base lg:text-lg text-gray-200">
          WUW hadir untuk membantu kalian yang ingin menyewa untuk beberapa hari saja pakaian yang akan kalian gunakan di hari terbaik kalian.
        </p>
        <div class="pt-4 flex items-center">
          <a href="#produk" class="inline-flex items-center bg-white text-blue-900 font-bold px-6 py-3 rounded shadow hover:bg-gray-500 hover:text-blue-300 transition duration-300">
            Temukan Pakaian 
          </a>
          <i class="ml-4 fa-solid fa-chevron-right fa-2xl"></i>
        </div>
      </div>

      <!-- Konten Kanan (Gambar) -->
      <div class="relative w-full max-w-md mx-auto lg:mx-0">
        <img src="img/15.png" alt="Pakaian" class="w-full relative z-10">
        <img src="img/16.png" alt="Pakaian" class="w-full absolute bottom-0 left-0 z-0">
        <img src="img/17.png" alt="Pakaian" class="w-full absolute top-0 left-0 z-0">
      </div>

    </div>
  </section>